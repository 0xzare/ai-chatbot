...
import { getUsers, getChatsCount, getMessagesCount } from '@/lib/db/queries';
...
export default async function AdminDashboard({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const statsData = [
    { name: t('users'), value: users.length },
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{t('growthMessages')}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <UserManagement users={users.map(u => ({ id: u.id, email: u.email, role: u.role }))} />
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <ul className="text-sm space-y-1">
                  <li><span className="font-medium">{t('mode')}:</span> {process.env.NODE_ENV || 'development'}</li>
                  <li><span className="font-medium">{t('database')}:</span> PostgreSQL</li>
                  <li><span className="font-medium">{t('authentication')}:</span> NextAuth.js</li>
                </ul>
              </div>
              <div>
                <h3 className="font-medium mb-2">{t('applicationInfo')}</h3>
                <ul className="text-sm space-y-1">
                  <li><span className="font-medium">{t('version')}:</span> 3.1.0</li>
                  <li><span className="font-medium">{t('framework')}:</span> Next.js 16</li>
                  <li><span className="font-medium">{t('uiLibrary')}:</span> shadcn/ui</li>
                </ul>
              </div>
            </div>
          </CardContent>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{t('projectDiscussion')}</p>
                </div>
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{t('aiModelUpdated')}</p>
                </div>
