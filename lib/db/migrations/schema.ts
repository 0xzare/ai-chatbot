import { pgTable, uuid, varchar, foreignKey, timestamp, text, boolean, json, numeric, primaryKey } from "drizzle-orm/pg-core"
...
export const messageKeyword = pgTable("MessageKeyword", {
	messageId: uuid().notNull(),
	keywordId: uuid().notNull(),
},
(table) => {
	return {
		messageKeywordMessageIdMessageV2IdFk: foreignKey({
			columns: [table.messageId],
			foreignColumns: [messageV2.id],
			name: "MessageKeyword_messageId_Message_v2_id_fk"
		}),
		messageKeywordKeywordIdKeywordIdFk: foreignKey({
			columns: [table.keywordId],
			foreignColumns: [keyword.id],
			name: "MessageKeyword_keywordId_Keyword_id_fk"
		}),
		messageKeywordMessageIdKeywordIdPk: primaryKey({ columns: [table.messageId, table.keywordId], name: "MessageKeyword_messageId_keywordId_pk"}),
	}
});
...
(table) => {
	return {
		...
		voteChatIdMessageIdPk: primaryKey({ columns: [table.messageId, table.chatId], name: "Vote_chatId_messageId_pk"}),
	}
});
...
(table) => {
	return {
		...
		voteV2ChatIdMessageIdPk: primaryKey({ columns: [table.messageId, table.chatId], name: "Vote_v2_chatId_messageId_pk"}),
	}
});
